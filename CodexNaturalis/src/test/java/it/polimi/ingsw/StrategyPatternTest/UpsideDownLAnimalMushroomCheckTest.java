...
public class UpsideDownLAnimalMushroomCheckTest {
    ...
    private void SetCards(int num){
        for(int i = 1; i <= num; i++){
            playedCards.put(new Point(i, -i), new ResourceCard(setFrontAngles(), SetbackAngles(), SetBack(), TypeOfCard.ANIMAL, true, 123, 0));
            playedCards.put(new Point(i, -i +2), new ResourceCard(setFrontAngles(), SetbackAngles(), SetBack(), TypeOfCard.ANIMAL, true, 123, 0));
            playedCards.put(new Point(i +1, -i+3), new ResourceCard(setFrontAngles(), SetbackAngles(), SetBack(), TypeOfCard.MUSHROOM, true, 123, 0));
        }
    }

    private void SetCardsMixed(int num){
        for(int i = -num*2 + 1; i <= num*2 -1; i = i +2 ){
            playedCards.put(new Point(1, i), new ResourceCard(setFrontAngles(), SetbackAngles(), SetBack(), TypeOfCard.ANIMAL, true, 123, 0));
            playedCards.put(new Point(2, i+1), new ResourceCard(setFrontAngles(), SetbackAngles(), SetBack(), TypeOfCard.MUSHROOM, true, 123, 0));
        }
    }
