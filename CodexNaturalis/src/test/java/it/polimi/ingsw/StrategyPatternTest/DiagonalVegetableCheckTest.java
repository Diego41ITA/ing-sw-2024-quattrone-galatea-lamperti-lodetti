...
import it.polimi.ingsw.model.card.strategyPattern.DiagonalVegetableCheck;
...
public class DiagonalVegetableCheckTest {
    ...
    public void SetUp() {
        ...
        goalCard = new GoalCard(0, true, 1, new DiagonalVegetableCheck(), new HashMap<>());
    }

    private void SetCards(int num) {
        for (int i = 1; i <= num; i++) {
            playedCards.put(new Point(i, -i), new ResourceCard(new HashMap<>(), new HashMap<>(), new ArrayList<>(), TypeOfCard.VEGETABLE, true, 123 + i, 0));
        }
    }
