...
public class PositionCheck5 implements CheckInterface{
    @Override
    public boolean check(HashMap<Point, PlayableCard> PlayedCard, HashMap<Item, Integer> AvailableItems, HashMap<Item, Integer> requirements) {
        for (Point a : PlayedCard.keySet()){
            if (PlayedCard.get(a).getType().equals(TypeOfCard.VEGETABLE)){
                if (PlayedCard.containsKey(new Point(a.x -1, a.y + 1)) && PlayedCard.get(new Point(a.x -1, a.y + 1)).getType().equals(TypeOfCard.MUSHROOM)){
                    if (PlayedCard.containsKey(new Point(a.x -1, a.y + 3)) && PlayedCard.get(new Point(a.x -1, a.y + 3)).getType().equals(TypeOfCard.MUSHROOM)){
                        return true;
                    }
                }
            }
